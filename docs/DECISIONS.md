...
## ⚠️ Rejected / Not Applicable (With Reasoning)
...
|----------|-------------|-----------|----------|
| **Remote monitoring** | Phone app shows real-time LDR values, battery %, servo position | Medium | ⭐⭐⭐ |
| **Remote control** | Override tracking, switch modes from phone | Medium | ⭐⭐ |
| **Configuration** | Adjust thresholds, calibrate remotely (no need to re-flash) | Low | ⭐⭐⭐ |
| **Data logging** | Stream LDR data to phone for analysis/graphing | Medium | ⭐⭐ |
| **Firmware OTA** | Update firmware wirelessly via BLE | High | ⭐ |
